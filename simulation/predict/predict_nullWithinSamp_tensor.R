...
# permute within samples # 02/26/2024
set.seed(1234)
for (i in unique(cellanno)) {
  expr.sample <- expr[, (cellanno == i)]
  pseudotime.sample <- pseudotime[cellanno == i]
  ###
  pseudotime.sample <- setNames(sample(pseudotime.sample), names(pseudotime.sample)) ## time-permuted!!!
  # expr.sample <- expr.sample[, sample(colnames(expr.sample))]
  # colnames(expr.sample) <- names(pseudotime.sample)
  ###
...
str(pseudotime.agg)
cellanno.agg <- sub(':.*', '', names(pseudotime.agg))
length(unique(cellanno.agg))
...
pseudotime_train = pseudotime.agg[cellanno.agg %in% sample_train] ## 02/26/24
pseudotime_test = pseudotime.agg[cellanno.agg %in% sample_test] ## 02/26/24
...
saveRDS(AUC.tensor, file = "simulation/v3/predict/AUC/AUC_nullWithinSamp_tensor.rds")
